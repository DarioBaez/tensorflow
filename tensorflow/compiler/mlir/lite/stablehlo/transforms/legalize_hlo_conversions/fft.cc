...
Value GetDimensionSizeTensor(OpBuilder& rewriter, Location loc, Value input,
                             int64_t dim) {
  auto sizeI32 = rewriter.create<mhlo::GetDimensionSizeOp>(loc, input, dim);
  return rewriter.create<mhlo::ReshapeOp>(
      loc, RankedTensorType::get({1}, rewriter.getI32Type()), sizeI32);
}
  ...
  LogicalResult matchAndRewrite(mhlo::FftOp fft_op,
    ...
    if (input_shape[input_shape.size() - 2] != 1) {
      ...
      if (is_dynamic_shape) {
        ...
        auto expanded_imput_shape_tensor = rewriter.create<mhlo::ConcatenateOp>(
            fft_op.getLoc(),
